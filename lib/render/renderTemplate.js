...
module.exports = function (inputs) {
  ...
  if (!templateName) {
    ;(async () => {
      try {
        ...
        console.log(table([['Name', 'Template Description']]))
        log.beauity(table(pkgs))
      } catch (err) {
        log.error('Network Error!')
      }
    })()
    return log.warn('Please select a template，format: fetp i <templateName> [project root directory]\n')
  }
  // 获取项目名称
  ...
  if (proLink && fsExists(proLink)) {
    inquirer
      .prompt([{
        type: 'confirm',
        message: `${projectName} directory already exists, overwrite？`,
        name: 'overwrite'
      }])
      .then(answer => {
        if (answer.overwrite) {
